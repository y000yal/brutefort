<?php
/**
 * Base Service for BruteFort plugin.
 *
 * @package BruteFort
 */
...
/**
 * Base Service class for common validation and sanitization.
 *
 * @package BruteFort
 */
class BaseService {
	...
	public function validate_and_sanitize_settings( array $data ): array {
		...
		foreach ( $data['formData'] as $field => $details ) {
			...
			$regex_pattern = 'brutef_ip_address' === $field ? '/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/' : '';
			...
			switch ( $type ) {
				...
				case 'checkbox':
					...
					$sanitized[ $field ] = in_array( $value, array( 'on', '1', 1, true ), true );
					break;
